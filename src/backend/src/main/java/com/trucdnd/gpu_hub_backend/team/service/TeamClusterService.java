...
public class TeamClusterService {
    ...
    public TeamClusterDto patch(UUID id, PatchTeamClusterRequest request) {
        TeamCluster teamCluster = getTeamCluster(id);
        ...
        if (request.teamId().isPresent()) {
            ...
            teamCluster.setTeam(team);
        }

        UUID clusterId = request.clusterId().orElse(teamCluster.getCluster().getId());
        UUID policyId = request.policyId().orElse(teamCluster.getPolicy().getId());
        if (request.clusterId().isPresent() || request.policyId().isPresent()) {
            ...
            teamCluster.setCluster(cluster);
            teamCluster.setPolicy(policy);
        }

        if (request.namespace().isPresent()) {
            teamCluster.setNamespace(request.namespace().orElse(null));
        }
    ...
    public void delete(UUID id) {
        teamClusterRepository.delete(getTeamCluster(id));
    }
