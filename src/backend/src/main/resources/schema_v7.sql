...
CREATE TABLE team_clusters (
    ...
    pvc_size    VARCHAR(20) NOT NULL DEFAULT '10Gi',
...
-- Trigger: data_source cluster phải nằm trong team_cluster của team
CREATE OR REPLACE FUNCTION check_data_source_team_cluster()
RETURNS TRIGGER AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM team_clusters
...
CREATE TRIGGER trg_data_source_team_cluster
BEFORE INSERT OR UPDATE ON data_sources
FOR EACH ROW EXECUTE FUNCTION check_data_source_team_cluster();

-- ============================================================
-- WORKLOAD_SOURCES
-- ============================================================
CREATE TABLE workload_sources (
    workload_id     	UUID NOT NULL REFERENCES workloads(id) ON DELETE CASCADE,
    source_id     		UUID NOT NULL REFERENCES data_sources(id) ON DELETE CASCADE,
    mount_path			VARCHAR(255) NOT NULL,
    PRIMARY KEY (workload_id, source_id)
);
...
CREATE INDEX idx_data_sources_team         ON data_sources(team_id);
CREATE INDEX idx_data_sources_cluster      ON data_sources(cluster_id);
